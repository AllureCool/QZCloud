...
import org.song.videoplayer.adapter.DefinitionAdapter;
 ...
 */
public class SuperVideoView extends QSVideoViewHelp {
    ...
    protected TextView switchDefinitionButton; //切换清晰度按钮
    ...
    public SuperVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        initView();
        setUIWithStateAndMode(STATE_NORMAL, currentMode, currentLockMode);
    }

    public void setOnPlayerControlListener(OnPlayerControlListener onPlayerControlListener) {
        mOnPlayerControlListener = onPlayerControlListener;
    }
    ...
    protected void initView() {
        ...
        switchDefinitionButton = (TextView) findViewById(R.id.help_clarity);
...
//        setiMediaControl(1);
        switchDefinitionButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if(mOnPlayerControlListener != null) {
                    mOnPlayerControlListener.clarityClick();
                }
            }
        });
    ...
    public void setDefinitions(List<DefinitionBean> definitions) {
        ...
        if(definitions != null && !definitions.isEmpty()) {
            switchDefinitionButton.setText(definitions.get(0).getDefinition());
        }
    }

    //根据状态设置ui显示/隐藏
    @Override
    protected void changeUiWithStateAndMode(int status, int mode, int lockMode) {
        switch (status) {
            case STATE_NORMAL:
            ...
            case STATE_AUTO_COMPLETE://显示底部 播放按钮 [顶部]
                showChangeViews(lockMode == MODE_UNLOCK ? startButton : null,
                        lockMode == MODE_UNLOCK ? bottomContainer : null,
                        lockMode == MODE_UNLOCK && mode == MODE_WINDOW_FULLSCREEN ? topContainer : null,
                        mode == MODE_WINDOW_FULLSCREEN ? lockButton : null);
                break;
            case STATE_ERROR://出错显示errorContainer
                showChangeViews(errorContainer);
                break;
        }
        updateViewImage(status, mode, lockMode);
    }

    //播放时隐藏的view
    @Override
    protected void dismissControlView(int status, int mode) {
        super.dismissControlView(status, mode);
        bottomContainer.setVisibility(View.INVISIBLE);
        topContainer.setVisibility(View.INVISIBLE);
        dismissDfinitionDialog();
...
//        progressBar.setVisibility(View.VISIBLE);
    ...
    protected void updateViewImage(int status, int mode, int lockMode) {
        if (status == STATE_PLAYING)
            startButton.setImageResource(R.drawable.jc_stop);
        else
            startButton.setImageResource(R.drawable.jc_play);

        if (lockMode == MODE_LOCKED)
            lockButton.setImageResource(R.drawable.jc_locked);
        else
            lockButton.setImageResource(R.drawable.jc_unlock);
    ...
    //清晰度选项弹窗
    protected PopupWindow mDefinitionWindow;
    protected RecyclerView mRvDefinition;

    protected boolean showDefinitionDialog() {
        if(mDefinitionWindow == null) {
            View localView = LayoutInflater.from(getContext()).inflate(R.layout.jc_dialog_definition, null);
            mRvDefinition = (RecyclerView) localView.findViewById(R.id.rv_definition);
            final LinearLayoutManager layoutManager = new LinearLayoutManager(getContext());
            mRvDefinition.setLayoutManager(layoutManager);
            final DefinitionAdapter adapter = new DefinitionAdapter(getContext());
            if(definitionList != null) {
                adapter.setDatas(definitionList);
            }
            adapter.setOnItemClickListener(new DefinitionAdapter.OnItemClickListener() {
                @Override
                public void onItemClick(View view, int position, DefinitionBean data) {
                    adapter.setSelectPosition(position);
                    switchDefinitionButton.setText(data.getDefinition());
                    dismissDfinitionDialog();
                    setUp(data.getVideoStream());
                    play();
                }
            });

            mRvDefinition.setAdapter(adapter);
            mDefinitionWindow = getPopupWindow(localView);
        }
        if(!mDefinitionWindow.isShowing()) {
            mDefinitionWindow.showAsDropDown(switchDefinitionButton, -220, 0);
            cancelDismissControlViewTimer();
        }
        mDefinitionWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            @Override
    ...
    protected boolean dismissDfinitionDialog() {
        if(mDefinitionWindow != null) {
            mDefinitionWindow.dismiss();
        }
        return true;
    }
    ...
    public static interface OnPlayerControlListener {
        void clarityClick();
    }
}
